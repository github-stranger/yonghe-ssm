...
<body>
	<h2>订单管理</h2>
	<div id="add-order">
		<a href="doorId" target="rightFrame">新增订单</a>
	</div>
	<hr/>
	<table border="1">
		<tr>
	...
	<tr>
			...
			<c:forEach items = "${doorlist }" var = "door">	
				<c:if test="${door.getId()==list.getDoor_id() }">
					<td>${ door.getName()}</td>
				</c:if>
			</c:forEach>
			<!--  -->
			<td>${list.getOrder_no() }</td>			
			<td>${list.getOrder_type() }</td>
			<td>${list.getPnum() }</td>
			<td>${list.getCashier() }</td>
			<td>
				
				<fmt:formatDate value="${list.getOrder_time() }"
			...
			<td>
				...
				<fmt:formatDate value="${list.getPay_time() }"
							pattern="yyyy/MM/dd HH:mm:ss" />
							
			</td>
			<td>${list.getPay_type() }</td>
			<td>${list.getPrice() }</td>
			<td>
				<a href="orderDelete?id=${list.getId() }">删除</a>
				&nbsp;|&nbsp;
				<a href="orderInfo?id=${list.getId() }">修改</a>
			</td>
		</tr>
		</c:forEach>
</table>
